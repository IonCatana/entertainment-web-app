...
const HomePage = () => {
  const [trendingItems, setTrendingItems] = useState(data.filter((item) => item.isTrending));
  const [recommendedItems, setRecommendedItems] = useState(data.filter((item) => !item.isTrending));
